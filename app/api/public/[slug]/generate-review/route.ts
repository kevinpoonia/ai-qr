...
export async function GET(
  ...
  const sql = await db();
  const rows = await sql`SELECT id, name, location FROM businesses WHERE slug = ${slug}`;
  const business = rows[0] as { id: number; name: string; location: string } | undefined;

  if (!business) {
    return NextResponse.json({ error: "Business not found" }, { status: 404 });
  }

  const index = await nextReviewIndexForBusiness(business.id);
